...
- 协同甲方完成数据对接与测试，优化数据刷新效率与准确率
- **凌志股份 金融BU（山西证券项目组）** 后端开发实习生
  202X.XX-202X.XX
  - 主导亿级证券客户数据处理，累计编写HQL代码1万+行，清洗/加工数据超2.3亿条，优化ETL流程后效率提升40%
  - **客户标签系统**：基于Hive构建客户资产与交易特征模型，开发Shell脚本实现T+1自动化打标（单日交易≥50W或总资产≥500W），支撑CRM精准营销，识别准确率99.2%
  - **客服奖金计算**：设计多阶段聚合HQL，实现客户达标判定与奖金阶梯计算（5-50W/50-100W/100W+分级加权），通过crontab每日自动化调度，计算时效从6小时缩短至45分钟

 **凌志股份有限公司  金融BU （山西证券项目组）**  *后端开发实习生*

参与多个亿级证券客户、员工数据处理，累计编写**Hive SQL**代码1万+行，清洗/加工数据超数亿条

**客户企微标签**：基于Hive构建客户资产与交易特征模型，开发Shell脚本实现T+1自动化打标，支撑CRM精准营销，30万+客户识别准确率100%

**客服奖金计算**：设计多阶段聚合HQL，实现客户达标判定与奖金阶梯计算（按交易额度分级加权），优化ETL流程，计算时效从6小时缩短至45分钟
...
**ETL** 是 **Extract, Transform, Load** 的缩写，翻译过来是**提取、转换、加载**，是数据处理的一个常见过程。ETL通常用于将数据从不同的源系统集成到一个集中式的数据仓库或数据湖中。这个过程包括三个主要步骤：

1. **Extract（提取）**：从不同的来源（例如数据库、文件、API等）提取数据。数据源可能是结构化的、半结构化的或非结构化的。
2. **Transform（转换）**：在提取数据之后，对数据进行清洗、格式化、去重、合并、计算等操作。这一步确保数据符合目标系统的要求，并且能够进行有效分析。
3. **Load（加载）**：将转换后的数据加载到目标数据存储系统（如数据仓库、数据库、数据湖等）中，供后续分析或应用使用。

ETL 过程的主要目的是确保数据从多个来源系统整合到一个统一的数据存储中，并能被高效地使用和分析。ETL工具（如Apache Nifi、Talend、Informatica等）可以自动化这些步骤，减少人工干预并提高效率。
...
使用一个较大的bit数组来保存所存的所有数据，每个元素都是1bit（只能是0或1）；

#### 1.1 存

当元素加入过滤器时，使用哈希函数对元素值进行计算，得到哈希值（多个函数对应多个哈希值），根据哈希值在数位组中把对应的下致为1；

#### 1.2 取

对给定元素再次进行相同的哈希值计算，若都为1则存在，一个为0则不存在；

**总结：**

布隆过滤器说元素存在，小概率误判，说不存在则一定不存在；

**Google开源的guava中自带布隆过滤器**
...
1. 在分库分表时会出现问题，当业务越来越大，分成多个表时，id就不唯一了；
2. 将自增id设置为不同的步长也不可以，当需要扩容时，如三个表变成四个表，数据量特别大；

##### 2.1.2 UUID  （隐私泄露，性能不高）

1. InnoDB索引结构不支持，InnoDB索引机构是一个B+树（索引即数据，数据即索引），UUID很长，所以B+树就很高，每次查询I/O的次数就会增多，影响性能；
2. UUID是无序的，非趋势递增的，（用到了MAC地址），所以每次插入数据，B+树都需要重新排序，同时也导致容易隐私泄露（用到了MAC地址）；
...
```java
now=time()  //获取当前时间戳
if now==last  // 如果当前时间等于上一时间戳生成的时间
    n++  	  // 让序列号+1
else n=0
//然后将符号位，时间戳，机器ID和序列号拼接起来
```
...
雪花算法使用了时间戳当时间不准确时，如时间快了，将时间回拨，就会出现ID不是区是递增的问题；

解决方案1：直接阻塞

解决方案2：将机器码划分三位给时间戳，生成时间序列
...
原文链接：[Typora-免费激活使用教程 - 简书](https://www.jianshu.com/p/2d8c40a4d93e)

备用链接：[https://blog.csdn.net/weixin_73609038/article/details/137751767](https://links.jianshu.com/go?to=https%3A%2F%2Fblog.csdn.net%2Fweixin_73609038%2Farticle%2Fdetails%2F137751767)
...
```
LicenseIndex.180dd4c7.bffb5802.chunk.js
```
...
onload事件名称的字符并不是标准的英文字母o和n，而是看起来像是使用了希腊字母或其他Unicode字符的相似字符（例如，ο是希腊字母omicron，而ο和o在视觉上非常相似）。这通常是一个错误，可能是复制粘贴时引入的。正确的应该是window.onload
...
查询所有key：
keys *
ps:redis是单线程的，在执行时就不能再执行其他命令，效率低不建议使用
...
```sql
先写时间：
setex name 10 weilin
<=> set name weilin ex 10
```
...
如果value是一个java对象，例如User对象，则可以将队形序列化为json字符串后储存：

| KEY              | VALUE                                |
| ---------------- | ------------------------------------ |
| weilin：user：1  | {"id":1,"name":"weilin","age":23}    |
| weilin:product:1 | {"id":1,”name“：”小米11“,"price":11} |

 redis可根据：来实现层级结构

```sql
set weilin:user:1 '{"id":1,"name":"weilin","age":23}'
set weilin:product:2 '{"id":2,”name“：”小米121“,"price":11}'
set weilin:user:2 '{"id":2,"name":"heima","age":23}'
```
...
|               | field | value |
| ------------- | ----- | ----- |
| weilin:user:1 | name  | jack  |
|               | age   | 21    |
| weilin:user:2 | name  | tom   |
|               | age   | 22    |

**命令如下：**

```sql
hset key field value：每次存一个key的一个field
hset weilin:user:3 name jack
hset weilin:user:3 age 21

hget key field :获取一个key的field
hget weilin:user:3 name

hmset key field value field value ...:每次存一个key的多个字段
hmset weilin:user:4 name lilei age 32 sex '男'

hmget key field field field:每次获取一个key的多个field
hmget weilin:user:4 name age  sex

hgetall key:获取一个key的所有field
hgetall weilin:user:4

hkeys key:获取一个key的所有field
hkeys weilin:user:4

hvals key:获取一个key的所有field的值
hvals weilin:user:4

hincrby key step:让一个key的一个field的value自增，并设置步长
hincrby weilin:user:4 age 2

hsetnx key field value:判断一个key的field是否存在，不存在则执行
hsetnx weilin:user:4 sex women
```

### 4. List类型命令

类似于LinkedList，元素可重复，插入删除快，查询速度一般

```sql
lpush key element:从左侧插入，可以插入多个
lpush user 1 2 3 4

lpop key count:从左侧取出，并设置取几个
lpop user 1

rpush key element:从右侧插入
rpush user 5 6 7 8

rpop key count:从右侧取出，并设置取几个
rpop user 1

lrange key star end:从star取到end
lrange user 3 7

blpop和brpop：取并且阻塞，等到设定时间结束
```

### 5. Set类型命令

类似HashSet,无序，元素不可重复，查找快，支持交集，并集差集等功能
...
redis中SortedSet是一个可排序的set集合，与java中的TreeSet有些类似，但底层数据结构差别很大。SrotedSet中的每一个元素中都带有一个score属性，可以根据score属性对元素排序，底层的实现是一个跳表（skipList）加hash表。
...
所有方法名都是redis命令，简单，但是只能单线程，多线程时要配置连接池
...
public class jedisTest {
    ...
    public void testString(){
        //插入数据
        String result=jedis.set("name","weilin");
...
public class JedisConnectionFactory {
    ...
    static {
        ...
        //创建连接池对象
        //配置信息 ip 端口号 超时时间 密码
        jedisPool=new JedisPool(jedisPoolConfig,"localhost",6379,2000,null);
    }
...
| redisTemplate.opsForZSet()  | ZSetOperations  |  ZSet  |
...
class RedisApplicationTests {
    ...
    void contextLoads() {
        //存入
        redisTemplate.opsForValue().set("name","weilin");
...
在java中，字符会被序列化，导致key和value变化，解决方案是不使用JDK默认的序列化工具
...
class RedisApplicationTests {
    ...
    void contextLoads() {
        ...
        redisTemplate.opsForValue().set("user",new User("jack",18));
...
为了在反序列化的时候知道对象类型，JSON序列化时会将class类型写入json结果中，会带来额外的开销

```java
{"@class":"com.weilin.redis.pojo.User","name":"jack","age":18}
```
...
**缓存穿透：**查询一个数据库中不存在的数据，mysql查询不到数据也不会直接写入缓存，就会导致每次请求都查询数据库；

- **解决方案1：**缓存空数据，查询数据为空，仍把这个数据结果进行缓存；

  - **缺点：**消耗内存，可能会发生不一致的问题
- **解决方案2：**布隆过滤器（可以使用redis或者Guava）
  - **缺点：**容易误判

#### 1.2 缓存击穿

**缓存击穿：**给某一个key设置了过期时间，当key过期时，恰好这个时间点对这个key有大量的并发请求过来，这些并发的请求也可能把DB击穿；

- **解决方案1：**互斥锁（强一致），当查询未命中时，加一个锁；
  - **缺点：**性能很低

- **解决方案2：**逻辑过期（高可用），加一个时间字段，查询时比对时间发现数据过期，就加一个互斥锁，并新建线程查询数据重建缓存数据，返回过期数据；
  - **缺点：**结果不一定一致；

#### 1.3 缓存雪崩

**缓存雪崩：**同一时间段内大量的key同时过期，或redis服务宕机，导致大量请求到达数据库，带来巨大压力；

- **解决方案1：**给不同的key的TTL添加随机值；
- **解决方案2：**利用redis集群提高服务的可用性（哨兵模式、集群模式）
- **解决方案3：**给缓存添加降级限流策略（ngxin）（保底解决方案，什么都可以解决）
- **解决方案4：**给业务添加多级缓存（布隆过滤器、Guava）

#### 1.4 双写一致性

**双写一致性：**当修改了数据库中的数据同时也要更新缓存的数据，缓存和数据库要保持一致性

- **解决方案1：**延时双删

  读操作 ：缓存命中，直接返回，未命中查询数据库，写入缓存，设置超时时间；

  写操作：延时双删，先删除缓存，再删除数据库，延时一会再删除一次缓存；
...
- **解决方案2：**分布式锁（强一致）
  ...
  排他锁：写锁writeLock，加锁之后，限制其他线程读写操作；

### 2.Spring

#### 2.1Spring框架中的单例bean是线程安全的吗？

**注意：**@Scope注解可以设置是否单例，默认为：@Scope（“singleton”）单例，@Scope（“prototype”）一个bean的定义可以有多个实例

Spring bean并没有可变的状态如（Service类和DAO类）所以在某种程度上说Spring单例bean是线程安全的。

**注意：**不可以修改的就是无状态的

**回答：**Spring中可修改的，如我在类中定义了一个成员变量，每次调用时都会修改这个成员变量，这个变量就是有状态的，他是线程不安全的；

#### 2.2 AOP

**AOP：**面向切面编程，用于将那些与业务无关，但却对多个对象产生影响的公共行为和逻辑，抽取并封装为一个可重用的模块，这个模块被称为“切面”（Aspect），减少系统中的重复代码，降低了模块间的耦合度，同时提高了系统的可维护性。

常见的AOP的使用场景：

- 记录操作日志
  - 使用aop中的环绕通知+切点表达式（找到要记录的日志的方法），通过环绕通知的参数获取请求方法的参数（类，方法，注解，请求方式等），获取到这些参数以后，保存到数据库
- 缓存处理
- Spring内置的事务处理

**事务：**一系列的操作，这些操作要么完全地执行，要么完全地都不执行， 它是一个不可分割的工作执行单元。
...
-  隔离性:通常来说，一个事务所做的修改操作在提交事务之前，对于其他事务来说是不可见的。（在前面的例
  子中，当执行完第三条语句、第四条语句还未开始时，此时有另外的一个账户汇总程序开始运行，则其看到支票帐户的余额并没有被减去200美元。）
-  持久性:一旦事务提交，则其所做的修改会永久保存到数据库。

#### 2.3 Spring的bean的生命周期

Spring容器在实例化时，会将xml配置的<bean>的信息封装成一个BeanDefinition对象，Spring根据BeanDefinition来创建Bean对象，里面有很多属性用来描述Bean

```xml
<bean id="userDao" class="com.weilin.dao.impl.UserDaoImpl"lazy-init="true"></bean>
<bean id="userService" class="com.weilin.service.UserServiceImpl" scope="singleton">
...
【建议4】单个方法的有效代码长度不宜超过100行（不包括有效长度）。

【建议5】单个类的长度包括注释行不宜超过1000行。

【规则2】方法间必须有2-3个空行。

【规则3】逻辑语句块间必须有一个空行，连续的语句之间不允许留空行。
...
【规则6】if，for，while等关键字之后应留一个空格再跟左括号“（”，以突出关键字。

【规则7】二元操作符前后必须加空格，一元操作符与其作用的变量之间禁止加空格。

【建议6】程序块的分隔符“{”宜和上一条语句位于同一行，以空格为分隔符。“}”宜独占一行，同时与引用他们的语句左对齐。

【建议7】不同语句的层次，宜使用限定4个空格键的tab键隔开。

【建议9】当方法参数过多时，可在每个参数后（逗号后）换行并对齐。

【规则11】一行的长度不超过80个字符，超过必须拆分出另一行。长表达式要在低优先级操作符处划分新行，**操作符放在新行之首**，划分出的新行要进行适当的缩进，使排版整齐，语句可读。

【建议13】修饰词宜按照指定顺序书写:[访问权限] [static] [final]。

【规则13】访问权限修饰词必须在最前面。

【建议15】源程序注释量宜在20%以上。

【建议19】避免在一行代码或表达式中间插入注释。

【建议20】对关键变量的定义和分支语句（条件分支、循环语句等）宜编写注释。

【建议28】类或接口头的注释宜放在class或者interface关键字之前，import关键字之后。

【规则14】命名中多个英文单词之间必须以首字母大写区分，或首字母小写加下划线。

【规则15】禁止使用仅靠大小写区分的相似命名。

【规则17】文件名要求每个单词必须以大写开头，使用多个英文单词组合，后缀表示文件类型。

【规则18】文件名总长度不超过50个字符。

【建议33】对外提供或与其它工程共用的文件，其命名前可以增加项目子系统名称（CSCI、CSC标识），即<CSCI|CSC>。

【建议34】包名采用域后缀倒置的加上自定义的包名，采用小写字母，不要出现“_”等字符。

```java
package com.cetc28.gcs.judgment
```

【规则19】类名和接口名的命名使用意义完整的英文描述，每个英文单词的首字母大写，其余字母使用小写的大小写混合法。接口的命名同时应在起始位置加上I。

【建议35】接口的实现类，单词宜以大写英文字母开头，由一个或多个单词组成，在结尾处加入Impl。

```java
class StateManage {};
interface IDataStorageClient{};
class DataStorageClientImpl implements IDataStorageClient{}  //接口实现类名
```

【建议36】常用组件类的命名宜以组件名加上组件类型名结尾。

```java
Application类型的，命名以App结尾--MainApp
Frame类型的，命名以Frame结尾--TopoFrame
Panel类型的，命名以Panel结尾--CreateCircuitPanel
Bean类型的，命名以Bean结尾--DataAccessBean
EJB类型的，命名以EJB结尾--DBProxyEJB
Applet类型，命名以Applet结尾--PictureShowApplet
```

【建议37】抽象类命名使用Abstract或Base开头；异常类命名使用Exception结尾；测试类命名以它要测试的类的名称开始，以Test结尾。

【建议38】枚举类名建议带上Enum后缀，枚举成员名称需要全大写，单词间用下划线隔开。

【建议39】如果模块、接口、类、方法使用了设计模式，在命名时体现出具体模式。
...
【规则20】方法的名字，使用意义完整的英文描述开头，单词首字母消息，后续单词首字母大写，使用“动词”或者“动词+名词”（动宾词组）。

【建议40】对于Service和DAO类，暴露服务应为接口，内部实现类命名建议使用Impl做后缀以与接口区分。

【规则21】方法中，存取属性的方法采用setter和getter方法，采用格式：get+非布尔属性名（），is+布尔属性名（），set+属性名。

```java
public String getType(); //获取非布尔属性
public boolean isFinished(); //获取布尔属性
public void setVisible(boolean); //设置布尔属性
public void setName(String Name); //设置非布尔属性
```

【建议42】在方法名中，应该省略其操作的类型名，避免不必要的重复；

```java
line.getLength(); //不建议 line.getLineLength();
```

【建议43】如果方法名超过15个字母，可采用以去掉元音字母的方法或者行业内约定俗成的缩写方式缩写方法名。

```java
public getFileSystemInfomation(){
    ......
}
//应修改为
public getFileSysInfo(){
    ......
}
```

【规则22】常量名使用全大写的英文描述，英文单词之间使用下划线分隔开，并且使用static final 修饰。

```java
public static final String BUSSINESS_RULES="bsl" //常量
```

【规则23】变量名使用意义完整的英文描述，第一个单词的字母使用小写，剩余单词首字母大写其余字母使用大小写混合法、变量名不能与方法名相同。

【建议44】变量名长度不宜超过30个字符。

【建议45】同一方法中一个局部变量的含义应保持一致，不同方法中含义相同的局部变量尽量使用同一命名。

【建议46】含有集合意义的属性命名，尽量包含其复数的意义。
...
【建议47】浮点型值应该至少保证小数点前后有一位有效数字。

```java
double total = 0.0; //错误写法 double total = 0;
double speed = 3.0e8; //错误写法 double speed = 3e8;
double sum = 0.5; //错误写法 double sum = .5;
```

【建议48】long或者Long初始赋值时，使用大写的L，不能是小写的l，易于数字1混淆。 

【建议49】常量宜使用关键字final来定义，定义后值不能改变。

【规则25】如果一个类的多个方法需要使用常量，必须在常量前加入static，并放在main方法外部。

【建议50】避免使用不易理解的数字，用有意义的标识符来代替。涉及物理状态或者含有物理意义的常量，不应直接使用数字，应尽量使用有意义的静态变量或者枚举来代替。

【建议51】尽量使用含义直观的常量来表示那些在程序中多次出现的数字或字符串，避免硬编码。

【建议52】不要使用一个常量类维护所有的常量，应该按常量功能进行归类，分开维护。

【建议53】接口（Interface）中定义的常量和方法默认是public static final类型，因此不必要在额外添加public、static、final等修饰词。

【规则26】变量使用前必须初始化。

【建议54】数组声明的时候宜使用int[] index,而不要使用int index[]。
...
【规则27】禁止使用未声明的变量，禁止方法中定义了局部变脸而没有使用。

【建议56】避免在作用域嵌套情况下不同层次间的变量名重名。

【建议57】尽量构造仅有一个模块或者方法可以修改，创建的公共变量，而其余有关模块或方法只进行读取访问。

【建议64】在同一语句中多处使用某个变量时，不要在该语句中同时出现修改其值的操作。
...
```java
  public void SimpleTest3(){
        ...
        if(array[index]<array[++index]) {
            r = 2;
        }//该语句在判断的同时修改了index的值，可能引起错误。
    }
```

【建议65】不要写没有必要的向上强制转换或者默认转换，否则会浪费性能增加代码阅读难度。

【建议66】为了方便阅读和减少没有必要的额计算，判断语句不要使用“*==true”来判断为真。
...
【建议68】使用if-else语句对某操作进行后续处理时，建议把执行成功时的处理语句置于if中，把执行失败后的语句置于else中。

【规则31】if语句的条件表达式中禁止使用赋值操作。

【建议69】进行变量与常量相等对比时，宜将常量写在相等比较运算符的左侧。

【建议70】尽量避免在for循环过程中修改循环变量。

【规则32】for循环中的循环变量必须是局部声明的。

【规则33】仅循环控制变量才能出现在for循环条件语句中，for循环应对使用关系运算符作为终止。
...
【规则34】使用switch语句时，必须要有default分支，且不能只有default分支；每个case都应该通过break/return来终止。

【规则35】禁止switch语句中条件语句为空，switcht条件语句中的表达式禁止是逻辑表达式。

【建议74】switch语句中，所有的非空case语句和default语句都应该使用break。

【规则36】case后面必须是整数或是枚举常量，不能是字符串。

【建议85】返回有特殊意义的数字时，应该先将该数字组定义为常量，返回大写形式的常量名，增强程序的可读性。

【规则43】必须避免用实例变量引用静态变量或方法，应使用类名，实例成员不应该修改static字段。

```java
private static int count=0;
public void doSomething(){
   count++; //避免通过实例方法修改static字段
}
```

【建议87】数组复制使用System.arraycopy(*)。

【规则45】Arrays.asList()把数组转换成集合时，禁止使用其修改集合相关的方法。

【规则46】在调用的最高层，必须处理所有异常。

【规则47】捕获了异常，然后抛出新的异常，必须把原异常的信息全部包含在新异常中。

【规则48】对于多个异常情况，必须对应多个catch分别处理，禁止使用一个catch处理多个异常。

【规则50】所有的覆写方法，必须加@Override注解。

ps：加@Override可以准确判断是否覆盖成功。

【规则51】Object的equals方法容易抛空指针异常，应用常量或确定优质的对象调用equals。

【规则52】实现类的equals（Object o）方法必须进行参数类型检查。

【规则53】禁止使用equals()方法来测试Atomic操作类型的值。

ps:Atomic类未重写equals()方法，当使用equals()时，实际调用的Object的eaquls()方法，最终是判断对象的引用地址是否相等。正确写法是先获取Atomic类对应的value值，调用value值对应的equals()方法判断是否相等。

【建议96】final可提高程序响应效率，声明final的情况：

- 不需要重新赋值的变量，包括类属性，局部变量；
- 对象参数前加final，表示不允许修改引用的指向；
- 类方法确定不允许被重写；

【建议97】对象使用结束时，宜赋值为null，由JVM回收资源。

【建议98】抽象类和子类的两级结构，创建子类实例时，宜使用静态工厂方法构造：
...
public class ShapeFactory {
    public static Shape newInstance(ShapeType shapeType) {
        switch (shapeType) {
            case Circle:{
                return new Circle();
            }
...
public void testCreateShape() {
    Shape line= ShapeFactory.newInstance(ShapeType.Line);
    Shape circle= ShapeFactory.newInstance(ShapeType.Circle);
}
```

【建议100】当类的实例有必选和可选的很多成员构成时，使用构造方法会带来很多参数和表意混乱问题，宜用建造者（Builder）模式替换构造方法。
...
【建议103】在抽象类和接口都能满足类设计需要时候，由于java的子类只能继承一个父类，但能够实现多个接口，当存在明显的继承关系时，使用抽象类，其他情况，推荐使用接口。

【建议104】对于一个类存在大量常量时，宜把常量定义在常量接口中。

【建议105】不要使用循环将集合转化为数组，可以使用集合的toArray()方法，使得代码更简洁。
...
【建议107】集合中的数据如果不使用了应该及时释放，尤其是可重复使用的集合。否则由于集合保存了对象的引用，虚拟机的垃圾收集器就不会回收。
